<template>
  <div class="user-role-switcher" v-if="canSwitchRoles" :class="{ collapsed: !isExpanded }">
    <div class="switcher-header" @click="toggleExpanded" :title="isExpanded ? 'Collapse Debug Panel' : 'Expand Debug Panel'">
      <span class="debug-icon">🔧</span>
      <span v-if="isExpanded" class="debug-label">Debug Role Switcher</span>
      <span class="beta-label">BETA</span>
      <button @click.stop="hideRoleSwitcher" class="close-debug-btn" title="Close Role Switcher">×</button>
    </div>
    
    <div v-if="isExpanded" class="switcher-content">
      <div class="beta-warning">
        <strong>⚠️ BETA WARNING:</strong> This debug tool is not working correctly and should be used with caution. It may not properly switch roles or could cause unexpected behavior.
      </div>
...
export default {
  ...
  setup() {
    ...
    // Check if user can switch roles (admin users)
    ...
    const hideRoleSwitcher = () => {
      // Simple hide functionality - could be expanded
      console.log('Role switcher hidden')
    }
