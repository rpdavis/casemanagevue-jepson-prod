...
export default {
  ...
  setup() {
    ...
    const previewFile = async () => {
      ...
      const rateCheck = checkRateLimit('aeriesPreview', 5, 300000) // 5 previews per 5 minutes
      ...
      try {
        const rawData = await parseAeriesCSVAndPreview(file)
          ...
          Object.entries(row).forEach(([key, value]) => {
            ...
            if (typeof value === 'string') {
              ...
              if (!valueSecurityCheck.isSafe) {
                console.warn(`Security threat detected in Aeries data: ${valueSecurityCheck.threats.join(', ')}`)
    ...
    const importData = async () => {
      ...
      const rateCheck = checkRateLimit('aeriesImport', 2, 600000) // 2 imports per 10 minutes
      if (!rateCheck.allowed) {
        showStatus('Too many import requests. Please wait before importing again.', true)
        return
      }

      if (!confirm(`Import ${parsedData.value.length} Aeries records? This will update existing students with matching SSIDs.`)) {
        return
      }
      ...
      try {
        ...
        await importFilteredAeriesRecords(validRecords, selectedFields.value)
        showStatus(`✅ Successfully imported ${validRecords.length} Aeries records.`)
