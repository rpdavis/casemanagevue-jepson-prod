...
export async function showStudentEmailDialog(studentId) {
  ...
  function renderDialog() {
    ...
    // only enable if student has at-a-glance or BIP URL
    const hasDocs = !!(student.ataglance_pdf_url || student.bip_pdf_url);
    const docsLabel = create("label");
    docsLabel.style.display = "block";
    docsLabel.innerHTML = `
      <input
        type="checkbox"
        id="include-docs"
    ...
    sendBtn.onclick = e => {
      ...
      // build the body text, injecting doc URLs if requested
const includeDocs = document.getElementById("include-docs").checked;
  let body = "";
     if (includeDocs) {
      body += `Hi team!

Below are links to ${student.first_name} ${student.last_name}'s updated documents:

`;

       if (student.ataglance_pdf_url) {
         body += `At-A-Glance: ${student.ataglance_pdf_url}\n`;
       }
       if (student.bip_pdf_url) {
         body += `BIP:       ${student.bip_pdf_url}\n`;
       }
     } else {
       body = "See attached.";
     }
      // ← NEW: compute student initials
      const initials =
        ((student.first_name?.[0] || "") + (student.last_name?.[0] || ""))
          .toUpperCase();

      // ← NEW: prefix initials to subject
      const subject = `${initials} - Student Update`;

      // ← UPDATED: use our new subject and body
      const gmailUrl =
